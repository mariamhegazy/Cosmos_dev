...
def main(args):
    ...
    files = glob.glob(os.path.join(args.dataset_path, "*.mp4")) + glob.glob(os.path.join(args.dataset_path, "*.webm"))
    ...
    start_idx = files_per_gpu * (rank) + min(rank, remainder)
    end_idx = start_idx + files_per_gpu + (1 if rank < remainder else 0)

    files = files[start_idx:end_idx]

    print(f"Processing {len(files)} files on rank {rank}, from {start_idx} to {end_idx}")
    ...
    with torch.no_grad():
        for video_path in tqdm(files):
            ...
            if not video_streams:
                log.info(f"Skipping {video_path} (no video stream found).")
                continue
            ...
            chunk_duration_sec = chunk_duration / fps  # Convert chunk duration to seconds
            num_chunks = int(duration_sec // chunk_duration_sec)

            print(f"Processing video {video_path} with duration {duration_sec} seconds and {num_chunks} chunks.")

            if duration_sec < chunk_duration_sec:
                log.info(f"Video {video_path} is shorter than {chunk_duration_sec} seconds. Skipped.")
                continue

            # Sample random segments
            cnt = 0 
            for _ in range(num_chunks):
                start_time = random.uniform(0, duration_sec - chunk_duration_sec) 
                ...
                if T >= chunk_duration:
                    video = video[:chunk_duration]
                else:
                    log.info(f"frames extracted from video {video_path} is shorter than {chunk_duration} frames. Skipped.")
                    continue
                # Rearrange dimensions: (T, H, W, C) -> (T, C, H, W)
                chunk = rearrange(video, "t h w c -> t c h w")
                ...
                # Save data to folder
                video_name = os.path.basename(video_path).split("/")[-1].split(".")[0]

                torch.save(latent[0], os.path.join(args.output_path, f"{cnt}-{video_name}.video_latent.pth"))
                torch.save(conditioning_latent[0], os.path.join(args.output_path, f"{cnt}-{video_name}.conditioning_latent.pth"))
                ...
                with open(os.path.join(args.output_path, f"{cnt}-{video_name}.info.json"), "w") as json_file:
