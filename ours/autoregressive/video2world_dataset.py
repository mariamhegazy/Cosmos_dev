...
class CosmosVideo2WorldDataset(Dataset):
    def __init__(self, data_path, model_config, split="train"):
        ...
        self.paths = [name for name in os.listdir(f"{self.data_path}")]
        

        metadata = {}
        metadata["train_samples"] = len(self.paths)
        metadata["test_samples"] = metadata["train_samples"]
        metadata["val_samples"] = metadata["train_samples"]
    ...
    def __getitem__(self, idx):
        ...
        #dummy prompt embedding
        prompt_embedding = torch.zeros(512, 1024).to(torch.bfloat16)
        video_tokens = torch.load(f"{self.data_path}/{self.paths[idx]}").to(
            torch.bfloat16
        )
